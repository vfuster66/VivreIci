...
export default function AdminReportsPage({
  ...
  const [currentPage, setCurrentPage] = useState(
    initialData?.meta?.currentPage ?? 1,
  );
  ...
  async function handleCreateReport(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setCreateError(
        error instanceof Error
          ? error.message
          : "Impossible de créer ce signalement.",
      );
    } finally {
      setIsCreating(false);
    }
        ...
        <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="rounded-full bg-[#F8FBFF] px-4 py-2 text-sm text-[#5B6572]">
              {data.meta?.totalMatchingReports ?? filteredReports.length}{" "}
              signalement(s)
            </div>
    ...
    >
      <section className="rounded-[32px] border border-[#0337AA]/8 bg-[linear-gradient(180deg,#ffffff_0%,#f4f8ff_100%)] p-6 shadow-[0_18px_48px_rgba(3,55,170,0.08)]">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          ...
          <p className="max-w-2xl text-sm leading-6 text-[#5B6572]">
            Utilise ces raccourcis pour passer d’une lecture globale à une
            action immédiate sur les signalements les plus sensibles.
          </p>
        </div>
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-2 text-sm leading-6 text-[#5B6572]">
                    {item.description}
                  </p>
                </div>
      ...
      {data.meta ? (
        <div className="mt-6 rounded-[24px] border border-[#FAC411]/35 bg-[#FFF8DF] px-4 py-4 text-sm leading-6 text-[#7B5B00]">
          ...
          {data.meta.totalMatchingReports} correspondant aux filtres. Les
          statistiques de synthèse restent calculées sur l&apos;ensemble du
          périmètre.
        </div>
      ) : null}
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-6">
        ...
        />
        <AdminMetricCard
          label="Visibles carte"
          value={data.stats.visibleOnMap}
        />
      </div>
      ...
      <AdminPanel className="mt-8">
        <div aria-live="polite" aria-atomic="true" className="mb-4 space-y-2">
          {feedbackMessage ? (
            <p
              className="rounded-2xl bg-[#ECFDF3] px-4 py-3 text-sm text-[#027A48]"
              role="status"
            >
              {feedbackMessage}
            </p>
            ...
            <AdminRecordCard key={report.id}>
              <div className="grid gap-4 xl:grid-cols-[1.5fr_0.8fr_1fr_1fr_0.9fr_0.9fr] xl:items-start">
                <div>
                  ...
                  <p className="mt-2 line-clamp-2 text-sm text-[#425166]">
                    {report.description}
                  </p>
                ...
                <div>
                  <p className="text-xs uppercase tracking-[0.18em] text-[#6B7280] xl:hidden">
                    Type
                  </p>
                ...
                <div>
                  ...
                  <div className="mt-1">
                    <span
                      className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${getReportStatusClasses(report.status)}`}
                    >
                  ...
                  {report.duplicateCount > 1 ? (
                    <p className="mt-2 text-xs text-[#92400E]">
                      {report.duplicateCount} doublons probables
                    </p>
                  ) : null}
                </div>
                <div>
                  ...
                  <div className="mt-1">
                    <span
                      className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${getConfidenceClasses(report.confidenceLevel)}`}
                    >
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <Link
                    href={buildCitizenAppUrl(
                      `/carte?lat=${report.lat}&lng=${report.lng}`,
                    )}
                    target="_blank"
                    rel="noreferrer"
