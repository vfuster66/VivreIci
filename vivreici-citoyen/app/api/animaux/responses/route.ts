...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === "create") {
      ...
      const { data: post, error: postError } = await supabase
        .from("lost_pets")
        .select("id, pet_name, animal_type, user_id, is_found")
        .eq("id", body.postId)
        .maybeSingle()
      ...
      if (post.is_found) {
        return NextResponse.json(
          { error: "Cette annonce est déjà clôturée." },
          { status: 400 }
        )
      }
      ...
      await createNotifications([
        {
          userId: post.user_id,
          type: "animal_post_response_received",
          title: "Nouvelle piste sur votre annonce animale",
          message: `${responderLabel} vous a écrit à propos de « ${post.pet_name?.trim() || post.animal_type || "votre annonce"} ».`,
          link: `/animaux?view=mine&postId=${post.id}`,
          metadata: {
            postId: post.id,
            responseId: data.id,
    ...
    if (body.action === "accept") {
      const { data: responseRow, error: responseError } = await supabase
        .from("lost_pet_responses")
        .select(
          "id, pet_id, post_owner_user_id, responder_label, responder_avatar_url, contact_email, contact_phone"
        )
      ...
      await createNotifications([
        {
          userId,
          type: "animal_post_contact_unlocked",
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Impossible de traiter cette réponse animale.",
      },
      { status: 500 }
    )
  }
