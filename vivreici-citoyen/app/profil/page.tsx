...
export default function ProfilPage() {
  ...
  const [animalAlertNotifications, setAnimalAlertNotifications] = useState(true)
  const [animalAlertsHighPriorityOnly, setAnimalAlertsHighPriorityOnly] =
    useState(false)
  ...
  useEffect(() => {
    ...
    setAnimalAlertNotifications(profile?.animal_alert_notifications ?? true)
    setAnimalAlertsHighPriorityOnly(
      profile?.animal_alerts_high_priority_only ?? false
    )
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        animal_alert_notifications: animalAlertNotifications,
        animal_alerts_high_priority_only: animalAlertsHighPriorityOnly,
            ...
            >
              <div className="space-y-3">
                ...
                <label className="flex items-start gap-3 rounded-2xl bg-[#F8F8F8] px-4 py-3 text-sm text-[#1A1A1A]">
                  <input
                    type="checkbox"
                    checked={animalAlertNotifications}
                    onChange={(event) =>
                      setAnimalAlertNotifications(event.target.checked)
                    }
                    className="mt-0.5"
                    disabled={!inAppNotifications}
                  />
                  <span>
                    Être prévenu des alertes animales proches.
                    <span className="mt-1 block text-xs leading-5 text-[#666666]">
                      Alertes localisées et risques du moment autour de votre ville suivie.
                    </span>
                  </span>
                </label>
                ...
                <label className="flex items-start gap-3 rounded-2xl bg-[#F8F8F8] px-4 py-3 text-sm text-[#1A1A1A]">
                  <input
                    type="checkbox"
                    checked={animalAlertsHighPriorityOnly}
                    onChange={(event) =>
                      setAnimalAlertsHighPriorityOnly(event.target.checked)
                    }
                    className="mt-0.5"
                    disabled={!inAppNotifications || !animalAlertNotifications}
                  />
                  <span>
                    Limiter aux alertes animales à forte vigilance.
                    <span className="mt-1 block text-xs leading-5 text-[#666666]">
                      Pour éviter le bruit si vous voulez seulement les risques les plus critiques.
                    </span>
                  </span>
                </label>
            ...
            >
              <div className="space-y-3">
                ...
                <div className="space-y-2 text-sm">
                  <p className="font-medium text-[#1A1A1A]">Documents légaux</p>
                  <ul className="space-y-2 text-[#666666]">
                    <li>
                      <MarketingLegalLink page="privacy" className="font-semibold text-[#D6A100]">
                        Politique de confidentialité
                      </MarketingLegalLink>
                    </li>
                    <li>
                      <MarketingLegalLink page="terms" className="font-semibold text-[#D6A100]">
                        Conditions générales d&apos;utilisation
                      </MarketingLegalLink>
                    </li>
                    <li>
                      <MarketingLegalLink page="cookies" className="font-semibold text-[#D6A100]">
                        Politique relative aux cookies
                      </MarketingLegalLink>
                    </li>
                    <li>
                      <MarketingLegalLink page="legal" className="font-semibold text-[#D6A100]">
                        Mentions légales
                      </MarketingLegalLink>
                    </li>
                  </ul>
              ...
              {reports.length === 0 ? (
                ...
                <div className="space-y-3">
                  {reports.map((report) => (
                    <Link
                      key={report.id}
                      href={`/signalements/${report.id}?from=profile`}
