...
function NouveauSignalementPageContent() {
  ...
  const [showAllDuplicateReports, setShowAllDuplicateReports] = useState(false)
  ...
  const hasLocationDetails =
    Boolean(selectedCoordinates) || address.trim().length > 0
  ...
  const completionSteps = [
    {
      ...
      detail: hasDescription
        ...
        : mediaFiles.length > 0
          ? "Média ajouté"
          : "Photo ou quelques mots",
      done: hasDescription || mediaFiles.length > 0,
    },
    {
      key: "locate",
      label: "Localiser",
      detail: hasLocationDetails ? effectiveLocationText : "Lieu à vérifier",
      done: hasLocationDetails,
    },
    {
      key: "review",
      label: "Préciser",
      detail: advancedOpen
        ...
        : effectiveType
          ? effectiveType
          : "Facultatif",
      done: advancedOpen || Boolean(selectedType),
    },
  ] as const
  ...
  const isSubmitDisabled =
    isSubmitting || isPreparingMedia || !isAuthenticatedUser
  const primaryDuplicateReport = duplicateReports[0] ?? null
  const visibleDuplicateReports = showAllDuplicateReports
    ? duplicateReports
    : duplicateReports.slice(0, 2)
  const submitHelperText = !isAuthenticatedUser
    ? "Vous pouvez tout préparer ici, puis vous connecter juste avant l'envoi."
    : !completionSteps[0].done
      ? "Ajoutez au moins une photo ou une courte description pour envoyer."
      : !isOnline
        ? "Sans réseau, le signalement sera gardé sur cet appareil puis repris automatiquement."
        : "Vous pourrez envoyer dès que la description ou un média est prêt."
  ...
  const advancedSummary = [
    selectedType ? `Type : ${selectedType}` : effectiveType ? `Type suggéré : ${effectiveType}` : null,
    address.trim() ? "Adresse précisée" : null,
    ...
    async function checkDuplicates() {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setShowAllDuplicateReports(false)
        }
      } catch {
        if (!cancelled) {
          setDuplicateReports([])
  ...
  const handleConfirmExistingReport = async (reportId: string) => {
    ...
    try {
      ...
      router.push(`/signalements/${reportId}?from=new`)
      router.refresh()
    } catch (error) {
  ...
  async function submitPayload(payload: PendingReportPayload) {
    ...
    try {
      ...
      router.push("/signalements?view=map")
      router.refresh()
    } catch (error) {
      ...
      <div className="mx-auto max-w-md overflow-hidden rounded-[30px] bg-white shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-5 pt-5 pb-6">
          ...
          {!isQueueReviewMode ? (
            ...
            <div className="rounded-[24px] border border-[#F1E4A6] bg-[#FFFDF2] px-4 py-4">
              <div className="flex items-start justify-between gap-3">
                <div className="space-y-1">
                  <p className="text-sm font-semibold text-[#A77D00]">
                    Parcours en 3 étapes
                  </p>
                  <p className="text-sm leading-6 text-[#5F5A45]">
                    Commencez par l&apos;essentiel. Les précisions restent
                    facultatives.
                  </p>
                </div>
                <span className="inline-flex min-h-10 min-w-10 items-center justify-center rounded-full bg-white px-3 text-sm font-semibold text-[#1A1A1A] ring-1 ring-[#F1E4A6]">
                ...
                {completionSteps.map((step, index) => (
                  <div
                    ...
                    className={`rounded-2xl px-3 py-3 ring-1 ${
                      step.done
                        ? "bg-white ring-[#F1E4A6]"
                        : "bg-[#FFF8D9] ring-[#F7E9AE]"
                    }`}
                  >
                    <p className="text-xs font-semibold uppercase tracking-[0.08em] text-[#A77D00]">
                      Étape {index + 1}
                    </p>
                    <p className="mt-1 text-sm font-semibold text-[#1A1A1A]">
                      {step.label}
                    </p>
                    <p className="mt-1 text-xs leading-5 text-[#5F5A45]">
                      {step.detail}
                    </p>
                  </div>
                ))}
              ...
              <p aria-live="polite" className="mt-3 text-xs leading-5 text-[#5F5A45]">
                {submitHelperText}
              </p>
            </div>

            <div className="rounded-[24px] bg-[#FBFBFB] p-4 ring-1 ring-gray-100">
              <div className="inline-flex items-center rounded-full bg-[#FFF7D6] px-3 py-1 text-xs font-semibold text-[#A77D00]">
                1. Décrire
              </div>
              ...
              <div className="mt-4 space-y-3">
                ...
                <div className="space-y-1 pt-1">
                  <div className="inline-flex items-center rounded-full bg-white px-3 py-1 text-xs font-semibold text-[#A77D00] ring-1 ring-gray-200">
                    2. Vérifier le lieu
                  </div>
                  <p className="text-sm leading-6 text-gray-500">
                    Une position approximative suffit pour démarrer. Vous pourrez
                    l&apos;affiner ensuite si besoin.
                  </p>
                </div>
            ...
            {!isCheckingDuplicates && primaryDuplicateReport ? (
              <div className="rounded-[24px] border border-[#F4C6C8] bg-[#FFF6F6] px-4 py-3.5">
                <div className="flex items-start gap-3">
                  <CircleAlert className="mt-0.5 h-5 w-5 shrink-0 text-[#d21c23]" />
                  <div className="min-w-0 space-y-1">
                    <p className="font-semibold text-[#1A1A1A]">
                      Un signalement similaire est déjà ouvert à proximité
                    </p>
                    <p className="text-sm leading-6 text-[#6B4A4D]">
                      Le plus proche est à {Math.round(primaryDuplicateReport.distanceMeters)} m.
                      Vérifiez-le avant d&apos;envoyer un nouveau signalement.
                    </p>
                  </div>
                </div>
              </div>
            ...
            {duplicateReports.length > 0 ? (
              <div className="space-y-3 rounded-[24px] border border-[#F4C6C8] bg-[#FFF6F6] p-4">
                <div className="flex items-start gap-3">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex flex-wrap gap-2 pt-1">
                          <button
                            type="button"
                            onClick={() => router.push(`/signalements/${report.id}?from=new`)}
                  ...
                  {duplicateReports.length > visibleDuplicateReports.length ? (
                    <button
                      type="button"
                      onClick={() => setShowAllDuplicateReports(true)}
                      className="inline-flex min-h-11 items-center justify-center rounded-full bg-white px-4 py-3 text-sm font-semibold text-[#1A1A1A] ring-1 ring-gray-200"
                    >
                      Voir les {duplicateReports.length - visibleDuplicateReports.length} autres suggestions
                    </button>
                  ) : null}
                </div>
              </div>
            ...
            <div className="rounded-[24px] border border-gray-200 bg-white">
              ...
              >
                <div>
                  <p className="font-semibold text-[#1A1A1A]">
                    3. Préciser si besoin
                  </p>
                  <p className="text-sm text-gray-500">
                    {advancedSummary || "Type, adresse précise et vidéo"}
                  </p>
                </div>
                {advancedOpen ? (
          ...
          {!isQueueReviewMode && offlineReports.length > 0 ? (
            <FeedbackBanner variant="info" className="space-y-3">
              <div className="flex items-start justify-between gap-3">
                <div>
                ...
                <div className="flex items-center gap-2">
                  <span className="inline-flex min-h-9 min-w-9 items-center justify-center rounded-full bg-white px-3 text-sm font-semibold text-[#1A1A1A] ring-1 ring-gray-200">
                  ...
                  <button
                    ...
                    disabled={isRetryingOfflineReports || !isOnline}
                    className="inline-flex h-9 items-center gap-2 rounded-full bg-white px-3 text-xs font-semibold text-[#1A1A1A] ring-1 ring-gray-200 disabled:opacity-50"
                  >
                    <RefreshCw
          ...
          {!isQueueReviewMode && mediaError ? (
            <FeedbackBanner variant="error" className="space-y-2">
              <p>{mediaError}</p>
              <button
                type="button"
                onClick={() => mediaLibraryInputRef.current?.click()}
          ...
          {!isQueueReviewMode ? (
            <div className="space-y-3 rounded-[24px] border border-gray-200 bg-white px-4 py-4">
              ...
              <div className="grid gap-2 sm:grid-cols-3">
                <div className="rounded-2xl bg-[#FBFBFB] px-3 py-3 ring-1 ring-gray-100">
                  <p className="text-xs font-semibold uppercase tracking-[0.08em] text-gray-500">
                    Description
                  </p>
                  <p className="mt-1 text-sm font-semibold text-[#1A1A1A]">
                    {hasDescription || mediaFiles.length > 0 ? "Prête" : "À compléter"}
                  </p>
                </div>
                <div className="rounded-2xl bg-[#FBFBFB] px-3 py-3 ring-1 ring-gray-100">
                  <p className="text-xs font-semibold uppercase tracking-[0.08em] text-gray-500">
                  ...
                  <p className="mt-1 text-sm font-semibold text-[#1A1A1A]">
                    {hasLocationDetails ? "Renseigné" : "Approx. ou manuel"}
                  </p>
                </div>
                <div className="rounded-2xl bg-[#FBFBFB] px-3 py-3 ring-1 ring-gray-100">
                  <p className="text-xs font-semibold uppercase tracking-[0.08em] text-gray-500">
                  ...
                  <p className="mt-1 text-sm font-semibold text-[#1A1A1A]">
                    {isAuthenticatedUser ? "Connecté" : "Connexion requise"}
                  </p>
                </div>
              </div>
